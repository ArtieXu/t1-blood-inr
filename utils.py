...
def visual_mag(img: torch.Tensor, path: str, max_value: Optional[float] = 0.6, nrow_num=6):
    ...
    img_grid = make_grid(
        normalization(torch.abs(img), max_value),
        nrow=nrow_num,
        pad_value=1,
...
def visual_gif(img: torch.Tensor, path: str, max_value: Optional[float] = 0.6):
    ...
    imgio.mimsave(
        path,
        [normalization(torch.abs(img), max_value)[i, ...].squeeze() for i in range(img.shape[0])],
        duration=0.1,
    )
...
def visual_err_mag(img1: torch.Tensor, img2: torch.Tensor, path: str, nrow_num=6, max_value=0.1):
    ...
    img_grid = make_grid(normalization(torch.abs(img1)), nrow=nrow_num, pad_value=2) - make_grid(
        normalization(torch.abs(img2)), nrow=nrow_num
    )
    mask = np.stack([(img_grid[0, :, :] == 2).cpu().numpy()] * 4, -1)
    jet_img = jet_cmap(
        np.clip(torch.abs(img_grid[0, :, :]).cpu().numpy(), 0, max_value) / max_value
    )
    jet_img[mask] = 1
...
def visual_err_phi(img1: torch.Tensor, img2: torch.Tensor, path: str, nrow_num=6, max_value=torch.pi / 6):
    ...
    mask = np.stack([(img_grid[0, :, :] == 2).cpu().numpy()] * 4, -1)
    jet_img = jet_cmap(
        np.clip(torch.abs(img_grid[0, :, :]).cpu().numpy(), 0, max_value / (2 * torch.pi))
        / max_value
        * (2 * torch.pi)
    )
    jet_img[mask] = 1
...
class TCurvLoss(torch.nn.Module):
    """L2 of the temporal second difference (dim 0 = frames).

    Penalizes curvature instead of slope, so a smooth near-monotonic relaxation
    curve is unpenalized while a per-frame staircase (aliasing) is. TVLoss (L1 of
    the first difference) cannot distinguish the two for a monotone sequence.
    """

    def __init__(self):
        super(TCurvLoss, self).__init__()

    def forward(self, x):
        d2 = x[2:, :, :, :] - 2 * x[1:x.shape[0] - 1, :, :, :] + x[:x.shape[0] - 2, :, :, :]
        return torch.sum(d2 ** 2) / x.numel()
...
class STVLoss(torch.nn.Module):
    ...
    def forward(self, x):
        return (
            torch.sum(torch.abs(x[:, :, 1:, :] - x[:, :, :x.shape[-2] - 1, :])) / x.numel()
...
class NUFFT:
    ...
    def forward(self, img):
        if img.shape[1] == 1:
            return self.nufft_op(img, self.ktraj, smaps=self.smap).reshape(
                [self.frame_num, self.coil_num, self.spoke_num, self.spoke_length]
            ) / self.grid_size
    ...
    def adjoint(self, kdata):
        return self.nufft_adj_op(
            kdata.reshape(self.frame_num, self.coil_num, -1) * self.dcomp.flatten(),
            self.ktraj,
            smaps=self.smap,
        ) / self.grid_size * torch.pi / (2 * self.spoke_num) / (torch.abs(self.smap) ** 2).sum(dim=0).unsqueeze(0).unsqueeze(0)


def metrics(imgs, gts, file_path=None):
    frames = imgs.shape[0]
    ...
    ssim = [structural_similarity(imgs[i, ...].squeeze(), gts[i, ...].squeeze(), data_range=1) for i in range(frames)]
